...
def sample_range(wav, sr, duration):
    ...
    return wav_sample
def load_mono_wavs(filenames, sr):
    return [librosa.load(filename, sr=sr, mono=True)[0] for filename in filenames]
...
def sample_data_batch(stfts_mono, stfts_src1, stfts_src2, batch_size=64, sample_frames=8):
    stft_mono_batch, stft_src1_batch, stft_src2_batch = [], [], []
    collection_idx = np.random.choice(len(stfts_mono), batch_size, replace=True)

    for idx in collection_idx:
        start = np.random.randint(stfts_mono[idx].shape[1] - sample_frames + 1)
        end = start + sample_frames
        stft_mono_batch.append(stfts_mono[idx][:, start:end])
        stft_src1_batch.append(stfts_src1[idx][:, start:end])
        stft_src2_batch.append(stfts_src2[idx][:, start:end])

    return np.array(stft_mono_batch).transpose((0, 2, 1)), \
           np.array(stft_src1_batch).transpose((0, 2, 1)), \
           np.array(stft_src2_batch).transpose((0, 2, 1))
